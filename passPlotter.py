def createPitch(length,width, unity,linecolor):
    ...
    return fig,ax
def plotter(fileName,MatchId,homeTeam):
    import matplotlib.pyplot as plt
    import numpy as np
    import pandas as pd
    pitchLengthX=120
    ...
    headers=["Team_Name","Player_Name","Xcor","Ycor","XcorEnd","YcorEnd","MatchId","Pass_Type","Cross","Cross_outcome","Length"]
    df.columns=headers
    df.dropna(subset = ["Team_Name"], inplace=True)
    cordinate=df.loc[:]
    #print(cordinate['Xcor'])
    for i,plot in cordinate.iterrows():
        if plot["MatchId"]<MatchId:
            continue
        if plot["MatchId"]>MatchId:
            break
        if plot["MatchId"]==MatchId:
            if plot["Team_Name"]==homeTeam:
                shotCircle=plt.Circle((pitchLengthX-int(plot['Xcor']),int(plot['Ycor'])),2,color="blue")
                shotCircle.set_alpha(.3)
            else:
                shotCircle=plt.Circle((pitchLengthX-int(plot['Xcor']),int(plot['Ycor'])),2,color="red")
                shotCircle.set_alpha(.3)
        ax.add_patch(shotCircle)
    fig.set_size_inches(10, 7)
    fig.savefig('c:/Users/dibya/Documents/WCpass.png', dpi=100) 
    plt.show()
plotter("ALLPass",8658,"France")
#plotter("OnlyGoals")    