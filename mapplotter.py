        ...
        if plot["MatchId"]==MatchId:
            if plot["Team_Name"]==HomeTeam:
                if plot["Outcome"]=="Goal":
                    shotCircle=plt.Circle((int(plot["Xcor"]),pitchWidthY-int(plot['Ycor'])),1,color="red")
                elif plot["Outcome"]=="Own Goal":
                    shotCircle=plt.Circle((int(plot["Xcor"]),pitchWidthY-int(plot['Ycor'])),1,color="red")
                else:
                    shotCircle=plt.Circle((int(plot["Xcor"]),pitchWidthY-int(plot['Ycor'])),2,color="blue")     
                    shotCircle.set_alpha(.4)    
            else:
                if plot["Outcome"]=="Goal":
                    shotCircle=plt.Circle((pitchLengthX-int(plot['Xcor']),int(plot['Ycor'])),1,color="red")
                elif plot["Outcome"]=="Own Goal":
                    shotCircle=plt.Circle((pitchLengthX-int(plot['Xcor']),int(plot['Ycor'])),1,color="red")
                else:
